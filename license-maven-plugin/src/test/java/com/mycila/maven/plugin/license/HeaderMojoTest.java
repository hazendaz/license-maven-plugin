...
class HeaderMojoTest {
  ...
  void test_create() throws Exception {
    ...
    check.defaultBasedir = Path.of("src/main/resources/check").toFile();
  ...
  void test_inlineHeader() throws Exception {
    ...
    LicenseCheckMojo check = new LicenseCheckMojo();
    check.defaultBasedir = Path.of("src/test/resources/check").toFile();
    check.legacyConfigInlineHeader = FileUtils.read(Path.of("src/test/resources/check/header.txt"), StandardCharsets.UTF_8);
    check.project = project;
    check.failIfMissing = false;
    check.strictCheck = true;
    check.execute();
  ...
  void test_multi_inlineHeader() throws Exception {
    ...
    multi.setInlineHeaders(new String[]{
        FileUtils.read(Path.of("src/test/resources/check/header.txt"), StandardCharsets.UTF_8),
        FileUtils.read(Path.of("src/test/resources/check/header2.txt"), StandardCharsets.UTF_8)
    });
    check.legacyConfigMulti = multi;
    check.project = project;
    check.failIfMissing = false;
